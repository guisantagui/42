...
static void shift_hue(double *norm_rgb, double *hsv, double hue_shift, int *is_error)
{
    double     *ifpqt;
...
static int color_shift_hue(int color, double hue_shift)
{
    ...
    free(hsv);
    if (is_error == 0)
        shift_col = create_trgb(0, (int)(col_hsv[2] * 255), (int)(col_hsv[1] * 255), (int)(col_hsv[0] * 255));
    else
        shift_col = -42;
    free(col_hsv);
    return (shift_col);
